...
echo "$($PSQL "SELECT winner.name AS name FROM games INNER JOIN teams AS winner ON games.winner_id = winner.team_id WHERE games.round = 'Eighth-Final' AND games.year = 2014 UNION SELECT opponent.name AS name FROM games INNER JOIN teams AS opponent ON games.opponent_id = opponent.team_id WHERE games.round = 'Eighth-Final' AND games.year = 2014 ORDER BY name")"
